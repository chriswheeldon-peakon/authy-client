...
function mock(_ref) {
  ...
  return (0, _nock2.default)(/\.authy\.com/).filteringPath(path => path.replace(/\/[0-9]+\//, '/{authyId}/')).post('/protected/json/users/{authyId}/remove', request.body).query(request.query ? request.query : true).reply(response.code, response.body);
}
